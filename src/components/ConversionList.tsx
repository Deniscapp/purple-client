...
const ConversionsList = () => {
    ...
    return (
      <>
        <StyledTypography variant="h4"  align="left">Most recent conversions</StyledTypography>
            ...
            <TableBody>
              {data?.conversions?.map((conversion: IConversion) => (
                <TableRow key={conversion._id}>
                  <TableCell>{new Intl.DateTimeFormat('en-GB').format(conversion.createdAt)}</TableCell>
                  <TableCell>{new Intl.NumberFormat('en-IN', { maximumSignificantDigits: 3 }).format(conversion.value)}</TableCell>
                  <TableCell>{conversion.originCurrency}</TableCell>
                  <TableCell>{conversion.targetCurrency}</TableCell>
                  <TableCell>{new Intl.NumberFormat('en-IN', { maximumSignificantDigits: 3 }).format(conversion.result)} {conversion.targetCurrency}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
