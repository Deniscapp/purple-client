...
import { StyledButton, Form, ConvertionWrapper, StyledTypography } from './styledComponents';
...
const ConversionCard = () => {
    ...
    const result = `${new Intl.NumberFormat('en-IN', { maximumSignificantDigits: 3 }).format(data?.newConversion?.result)} ${targetCurrency}`

    const areInputsFilled = !value || !originCurrency || !targetCurrency;

    return (
        <>
            <StyledTypography variant="h4" align="left">Currency converter</StyledTypography>
            <ConvertionWrapper>
                <Card>
                    <CardContent>
                        <Form autoComplete="off">
                            ...
                            />
                            <StyledButton
                                onClick={() => createConversion({ variables: { newConversion: { value, originCurrency, targetCurrency } } })}
                                variant="contained"
                ...
                <ResultCard loading={loading} value={data?.newConversion?.result ? result : 0} title="Result" />
            </ConvertionWrapper>
        </>
    );
}
